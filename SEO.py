...
try:
	...
	for idx,url in enumerate(list_URL[1:]):
		if idx != '' and idx != None and url != '' and url != None:
			print("\nprocessing [" + str(idx+1) + "/" + str(len(list_URL)-1) + "] --> " + url)
			...
			if soup.title == None:
				...
				rgb=[255,0,0]
				color_string="".join([str(hex(i))[2:].upper().rjust(2, "0") for i in rgb])
				sheet['D' + index].fill=PatternFill(fill_type="solid", start_color='FF' + color_string, end_color='FF' + color_string)
			...
			if count == 0:
				...
				sheet['F' + index] = 'MEHR ALS 2 H1-ÜBERSCHRIFTEN (' + str(count) + ' H1 FOUND)'
			...
			for a in soup.findAll("a", attrs={"href":True}):
				if len(a['href'].strip()) > 1 and a['href'][0] != '#' and 'javascript:' not in a['href'].strip() and 'mailto:' not in a['href'].strip() and 'tel:' not in a['href'].strip():
					if 'http' in a['href'].strip() or 'https' in a['href'].strip():
						if urlparse(sheet['B' + index].value).netloc.lower() in urlparse(a['href'].strip()).netloc.lower():
							intlink += 1
						else:
							extlink += 1
					else:
			...
			if intlink == 0:
				rgb=[255,0,0]
				color_string="".join([str(hex(i))[2:].upper().rjust(2, "0") for i in rgb])
				sheet['Q' + index].fill=PatternFill(fill_type="solid", start_color='FF' + color_string, end_color='FF' + color_string)
			else:
				rgb=[255,255,255]
				color_string="".join([str(hex(i))[2:].upper().rjust(2, "0") for i in rgb])
				sheet['Q' + index].fill=PatternFill(fill_type="solid", start_color='FF' + color_string, end_color='FF' + color_string)
			...
			if extlink == 0:
				rgb=[255,0,0]
				color_string="".join([str(hex(i))[2:].upper().rjust(2, "0") for i in rgb])
				sheet['R' + index].fill=PatternFill(fill_type="solid", start_color='FF' + color_string, end_color='FF' + color_string)
			else:
				rgb=[255,255,255]
				color_string="".join([str(hex(i))[2:].upper().rjust(2, "0") for i in rgb])
				sheet['R' + index].fill=PatternFill(fill_type="solid", start_color='FF' + color_string, end_color='FF' + color_string)
			...
			for ifr in soup.findAll('iframe', attrs={'src':True}):
				if 'youtube.com' in ifr['src']:
					...
					rgb=[255,255,255]
					color_string="".join([str(hex(i))[2:].upper().rjust(2, "0") for i in rgb])
					sheet['S' + index].fill=PatternFill(fill_type="solid", start_color='FF' + color_string, end_color='FF' + color_string)
					break
			...
			if len(li) == 0:
				rgb=[255,0,0]
				color_string="".join([str(hex(i))[2:].upper().rjust(2, "0") for i in rgb])
				sheet['T' + index].fill=PatternFill(fill_type="solid", start_color='FF' + color_string, end_color='FF' + color_string)
			else:
				rgb=[255,255,255]
				color_string="".join([str(hex(i))[2:].upper().rjust(2, "0") for i in rgb])
				sheet['T' + index].fill=PatternFill(fill_type="solid", start_color='FF' + color_string, end_color='FF' + color_string)
			...
			print("\nprocessing [" + str(idx+1) + "/" + str(len(list_URL)-1) + "] --> SKIPPING.. INVALID DOMAIN FOUND..")
			
	wb.save(filepath)
	end = datetime.now()
	print('\nprogram finished at ' + str(end))
	print('\ntotal time taken is ' + str((end - start).seconds) + '.' + str((end - start).microseconds) + ' seconds')
	...
	print('\nwebsite (' + url + ') not found, exiting program, correct URL and run program again\n')
